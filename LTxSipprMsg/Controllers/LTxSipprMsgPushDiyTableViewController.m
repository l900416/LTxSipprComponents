//
//  LTxSipprMsgPushDiyTableViewController.m
...
#import "LTxSipprMsgPushDiyTableViewController.h"
#import "LTxSipprMsgViewModel.h"

@interface LTxSipprMsgPushDiyTableViewController ()
...
@implementation LTxSipprMsgPushDiyTableViewController
...
-(void)pushTypePushListFetch{
    __weak __typeof(self) weakSelf = self;
    [LTxSipprMsgViewModel pushTypeListFetchComplete:^(NSArray *pushTypeList, NSString *errorTips) {
        __strong __typeof(weakSelf)strongSelf = weakSelf;
        strongSelf.dataSource = [pushTypeList mutableCopy];
        if (!errorTips && [pushTypeList count] == 0) {
            strongSelf.errorTips = @"尚未配置推送定制项";
        }else{
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    if (_selectedConfigSet) {
        [LTxSipprMsgViewModel diyPushTypeList:_selectedConfigSet complete:nil];
    }
}
...
-(UISwitch *)switchWithStatus:(BOOL)status tag:(NSInteger)tag{
    ...
    retSwitch.onTintColor = [LTxSipprConfig sharedInstance].skinColor;
