...
#import "LTxSipprCameraShootViewController.h"
#import "LTxSipprCameraShootToolView.h"
#import <AVFoundation/AVFoundation.h>
#import "LTxSipprVideoPlayerView.h"
#import "LTxSipprCameraUtil.h"
#define  LTXSIPPR_CAMERA_TOOL_VIEW_HEIGH 160

@interface LTxSipprCameraShootViewController ()<LTxSipprCameraShootToolDelegate,AVCaptureFileOutputRecordingDelegate>
...
//UI
@property (nonatomic, strong) LTxSipprCameraShootToolView* toolView;//控制区
...
@property (nonatomic, strong) LTxSipprVideoPlayerView *playerView;//播放视频


@end

@implementation LTxSipprCameraShootViewController
...
-(void)doneCallback{
    ...
    if (self.takedImage) {
        ...
        [LTxSipprCameraUtil saveImageToAblum:self.takedImage completion:^(BOOL success, PHAsset *asset) {
            if (success) {
    ...
    if (self.videoUrl) {
        [self showAnimatingActivityView];
        [LTxSipprCameraUtil saveVideoToAblum:self.videoUrl completion:^(BOOL success, PHAsset *asset) {
            if (success) {
                [weakSelf hideAnimatingActivityView];
                [weakSelf onDismiss];
            }else{
...
- (void)playVideo{
    if (!_playerView) {
        self.playerView = [[LTxSipprVideoPlayerView alloc] initWithFrame:self.view.bounds];
...
-(LTxSipprCameraShootToolView*)toolView{
    if (!_toolView) {
        _toolView = [[LTxSipprCameraShootToolView alloc] init];
